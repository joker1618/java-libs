package xxx.joker.apps.formula1.dataCreator.parsers.years;

import org.apache.commons.lang3.StringUtils;
import xxx.joker.apps.formula1.dataCreator.model.entities.*;
import xxx.joker.apps.formula1.dataCreator.model.fields.F1FastLap;
import xxx.joker.apps.formula1.dataCreator.parsers.AWikiParser;
...
public class Year2015 extends AWikiParser {
    ...
    protected void parseEntrants(String html) {
        ...
        for (JkTag tr : tbody.getChildren("tr")) {
            ...
            if(tdList.size() >= 7) {
                JkTag tagTeamName = tr.getChild(1).findChild("span b a", "b a");
    ...
    protected void parseQualify(String html, F1GranPrix gp) {
        ...
        for (JkTag tr : tbody.getChildren("tr")) {
            ...
            if(tr.getChildren("th").size() == 1 && (tdNum == 6 || tdNum == 7)) {
                ...
                q.getTimes().add(parseDuration(fixTime(tr.getChild(4).getTextFlat())));
                q.getTimes().add(parseDuration(fixTime(tr.getChild(5).getTextFlat())));

                int counter = 6;
                if(tdNum == 7) {
                    q.getTimes().add(parseDuration(fixTime(tr.getChild(counter++).getTextFlat())));
                }
